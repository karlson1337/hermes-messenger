...
void db_init() {
  ...
  sqlite3_exec(db,
               ...
               "(username TEXT PRIMARY KEY, pwhash TEXT NOT NULL, pubkey BLOB "
               "NOT NULL);",
               NULL, NULL, NULL);

  if (sqlite3_open("hermes_messages.db", &message_db) != SQLITE_OK) {
    fprintf(stderr, "sqlite open: %s\n", sqlite3_errmsg(message_db));
    exit(1);
  }
...
bool db_register(const char *username, const char *pwhash,
  ...
  sqlite3_stmt *stmt;
  sqlite3_prepare_v2(db, "INSERT INTO identities VALUES (?,?,?)", -1, &stmt,
                     NULL);
...
bool db_get_pwhash(const char *username, char *out_hash) {
  sqlite3_stmt *stmt;
  sqlite3_prepare_v2(db, "SELECT pwhash FROM identities WHERE username=?", -1,
                     &stmt, NULL);
  sqlite3_bind_text(stmt, 1, username, -1, SQLITE_STATIC);
...
bool get_pubkey(const char *username, unsigned char *pubkey_out) {
  sqlite3_stmt *s;
  sqlite3_prepare_v2(db, "SELECT pubkey FROM identities WHERE username=?", -1,
                     &s, NULL);
...
bool db_queue_push(const char *recipient, const unsigned char *content) {
  sqlite3_stmt *stmt;
  sqlite3_prepare_v2(message_db, "INSERT INTO messages VALUES (?,?)", -1, &stmt,
                     NULL);
  sqlite3_bind_text(stmt, 1, recipient, -1, SQLITE_STATIC);
  sqlite3_bind_blob(stmt, 2, content, MESSAGE_MAX + crypto_box_SEALBYTES,
                    SQLITE_STATIC);
  sqlite3_step(stmt);
  int changes = sqlite3_changes(db);
  sqlite3_finalize(stmt);
...
bool db_get_queued(const char *recipient,
  ...
  while (sqlite3_step(stmt) == SQLITE_ROW) {
    ...
    sqlite3_stmt *del;
    sqlite3_prepare_v2(message_db, "DELETE FROM messages WHERE rowid=?", -1,
                       &del, NULL);
