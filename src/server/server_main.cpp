...
void db_init();
bool db_register(const char *username, const char *pwhash,
                 const unsigned char *pubkey);
bool db_get_pwhash(const char *username, char *out_hash);
bool get_pubkey(const char *username, unsigned char *pubkey_out);

bool db_get_queued(const char *recipient,
                   void (*cb)(const unsigned char *content, void *ud),
                   void *ud);
...
bool authenticate(int sock, char username_out[USERNAME_SIZE]) {
  ...
  if (crypto_box_seal_open((unsigned char *)&payload, ciphertext, cipher_len,
                           server_pk, server_sk) != 0) {
  ...
  if (!db_get_pwhash(payload.username, stored_hash)) {
    ...
    if (crypto_pwhash_str(stored_hash, payload.password,
                          strlen(payload.password),
    ...
    if (crypto_pwhash_str_verify(stored_hash, payload.password,
                                 strlen(payload.password)) == 0) {
      ...
      printf("Auth failed (wrong password) for: %s\n", payload.username);
    }
  }
...
void *connection_handler(void *socket_desc) {
  ...
  while (1) {
    ...
    if (recv_all(sock, buf, MESSAGE_MAX + crypto_box_SEALBYTES) <= 0)
      break;
    ...
    if (buf[0] == TYPE_FRIENDADD) {
      ...
      crypto_box_seal((unsigned char *)temp + HEADER, temp_pk,
                      crypto_box_PUBLICKEYBYTES, sender_pk);
  ...
  while (1) {
    ...
    *new_fd = client_fd;
    ...
    pthread_attr_setstacksize(
        &attr,
        96 * 1024); // set stack size to 96kb (default 8mb is way too much)

    if (pthread_create(&listener_thread, &attr, connection_handler,
                       (void *)new_fd) < 0) {
      perror("error creating new thread");
      close(*new_fd);
      free(new_fd);
      if (errno == EAGAIN) {
        fprintf(stderr, "server might be under high load.\n");
      }
