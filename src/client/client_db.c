...
bool derive_db_key(const char *password, const uint8_t *salt,
                   char *hex_key_out) {
  uint8_t key[32];
...
bool read_key_salt(unsigned char *salt_out) {
  char path[256];
  snprintf(path, sizeof(path), "%s/.config/hermes/%s.key", getenv("HOME"),
           self);
...
bool chatdb_open(const char *password) {
  ...
  if (!read_key_salt(salt))
    return false;

  char hex_key[65];
  if (!derive_db_key(password, salt, hex_key))
    return false;

  char db_path[256];
  get_chatdb_path(self, db_path, sizeof(db_path));
  if (sqlite3_open(db_path, &chat_db) != SQLITE_OK)
    return false;
  char pragma[128];
...
bool chatdb_insert(const char *sender, const char *recipient, int64_t timestamp,
                   const unsigned char *content) {
  sqlite3_stmt *stmt;
  const char *sql = "INSERT INTO messages(timestamp,sender,recipient,content) "
                    "VALUES(?,?,?,?);";
...
bool chatdb_history(const char *user_a, const char *user_b, int limit,
                    int offset,
                    void (*cb)(int64_t ts, const char *sender,
...
int chatdb_get_count(const char *user_a, const char *user_b) {
  sqlite3_stmt *stmt;
  const char *sql = "SELECT COUNT(*) FROM messages WHERE (sender=? AND "
...
void friend_db_init() {
  char path[256];
  snprintf(path, sizeof(path), "%s/.config/hermes/%s_friends.db",
           getenv("HOME"), self);
  sqlite3_open(path, &friend_list);
  sqlite3_exec(friend_list,
               "CREATE TABLE IF NOT EXISTS friends "
               "(username TEXT PRIMARY KEY, pubkey BLOB NOT NULL, unread "
...
void friends_add(const char *username, const unsigned char *pubkey) {
  ...
  sqlite3_prepare_v2(friend_list,
                     "INSERT OR REPLACE INTO friends VALUES (?,?,?)", -1, &s,
                     NULL);
...
bool friend_get_pubkey(const char *username, unsigned char *pubkey_out) {
  sqlite3_stmt *s;
  sqlite3_prepare_v2(friend_list, "SELECT pubkey FROM friends WHERE username=?",
                     -1, &s, NULL);
  sqlite3_bind_text(s, 1, username, -1, SQLITE_STATIC);
...
bool friend_exists(const char *username) {
  sqlite3_stmt *s;
  sqlite3_prepare_v2(friend_list, "SELECT 1 FROM friends WHERE username=?", -1,
                     &s, NULL);
  sqlite3_bind_text(s, 1, username, -1, SQLITE_STATIC);
...
void friends_increment_unread(const char *username) {
  sqlite3_stmt *stmt;
  sqlite3_prepare_v2(friend_list,
                     "UPDATE friends SET unread = unread + 1 WHERE username=?",
                     -1, &stmt, NULL);
...
void friends_clear_unread(const char *username) {
  sqlite3_stmt *stmt;
  sqlite3_prepare_v2(friend_list,
                     "UPDATE friends SET unread = 0 WHERE username=?", -1,
                     &stmt, NULL);
  sqlite3_bind_text(stmt, 1, username, -1, SQLITE_STATIC);
  sqlite3_step(stmt);
  sqlite3_finalize(stmt);
